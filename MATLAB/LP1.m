...
for i=1:26 %a
    for j=1:26 %b
        ...
        y=a(i,:);
        y1=a(j,:);
        c0=(a([3 5 6 7 8],:)-min(a([3 5 6 7 8],:))).^0.473;
        if sum(y)==sum(y1)
            b=a(:,[1 2 3 6]);
            s=y(:,[1 2 3 6]);
            [~,ic]=ismember(b,s,'rows');
            rows=find(ic);
            ref=a(rows,:);
        else
            ref=0;
        end                   % find a reference set
        c1=([y;y1]-min(ref)).^0.473;
        c=[c0;c1];
        Z(7,15)=0;
        for k=1:7
            z=c(k,:);
...
%         zz=z2(6,1:21)-z2(7,1:21);
% w=0;
% for k=1:26
%     for t=1:26
%         AA=[3	5	7	6	8	9	26	20	11	4	10	12	23	22	14	15	25	24	16	21	13	1	2	17	18	19];
% %         AA1=[3   5  7  6  9 8  26  20  11  4  10  12  23  22  14  15  25  24  16  21  13  2  1  17 19 18];
%         if (AA(t)==k)
%             w=w+length(f(f(k,(AA(t:end)))>=0));
%         else
%         end
%         if (f(k,t)>=0)
%             [BB,index]=sort(AA);
...
%         AA=[3	5	7	6	8	9	26	20	11	4	10	12	23	22	14	15	25	24	16	21	13	1	2	17	18	19];
%         if (AA(t1)==k1)
%             w1=w1+length(f(f(k1,(AA(t1:end)))>=0));
%         else
%         end
%         if (f(k1,t1)>=0)
%             [BB,index]=sort(AA);
%             if (index(k1)>index(t1))
%                 h1=h1+1;
%             else
%             end
%         end
